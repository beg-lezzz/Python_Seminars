...
def check_choice(choice):
    check_choice(model.print_data_base()) if choice == '0-1' else \
    ...
    check_choice(model.edit_record(view.find_menu(choice))) if choice == '2-3' else \
    ...
    check_choice(model.export_data_base(choice)) if choice in ('4-2', '4-1') else \
    check_choice(model.import_data_base(choice)) if choice in ('5-2', '5-1') else \
    check_choice(view.main_menu()) if choice in ('2-0', '3-0', '4-0', '5-0', '0') else \
    quit("\033[32mСпасибо, что воспользовались нашей системой. Ждём Вас снова.\033[0m") if choice == '0-0' else \
